...
Vissim.Net.Links.SetAllAttValues(Attribute, Cost,
                                 True)  # setting the 3rd input to true, will add 5.5 to all previous costs!
...
State_of_SH = Vissim.Net.SignalHeads.ItemByKey(SH_number).AttValue(
    'State')  # possible output: 'GREEN', 'RED', 'AMBER', 'REDAMBER'
...
veh_linklanes = Vissim.Net.Vehicles.GetMultiAttValues(
    'Lane')  # Output 1. column:consecutive number; 2. column: AttValue
for cnt in range(len(veh_numbers)):
    print('%s  |  %s  |  %.2f  |  %.2f  |  %s' % (
        veh_numbers[cnt][1], veh_types[cnt][1], veh_speeds[cnt][1], veh_positions[cnt][1],
        veh_linklanes[cnt][1]))  # only display the 2nd column
...
lane_number = 1
link_coordinate = 70
Vehicle.MoveToLinkPosition(link_number, lane_number,
                           link_coordinate)  # This function will operate during the next simulation step
...
link = 1
lane = 1
x_coordinate = 15  # unit according to the user setting in Vissim [m or ft]
interaction = True  # optional boolean
new_Vehicle = Vissim.Net.Vehicles.AddVehicleAtLinkPosition(vehicle_type, link, lane, x_coordinate, desired_speed,
                                                           interaction)
...
# Alternatively, load a LayoutFile where dynamic elements (vehicles and pedestrians) are not visible:
...
print('Average travel time all time intervalls of all simulation of all vehicle '
      'classes: %.2f (number of vehicles: %s)' % (TT, No_Veh))
...
print('Maximum travel time of all time intervalls of the current simulation'
      ' of vehicle class HGV: %.2f (number of vehicles: %s)' % (TT, No_Veh))
...
#   DC_measurement.AttValue('Vehs(sub_attribute_1, sub_attribute_2, sub_attribute_3)')
#
# sub_attribute_1: SimulationRun (same as described at Vehicle Travel Time Measurements)
# sub_attribute_2: TimeInterval  (same as described at Vehicle Travel Time Measurements)
# sub_attribute_3: VehicleClass  (same as described at Vehicle Travel Time Measurements)
#
...
print('Data Collection #', DC_measurement_number, ': Average values of all Simulations runs of'
                                                  ' 1st time interval of all vehicle classes:')
print('#vehicles: %d; Speed: %.2f; Acceleration: %.2f; Length: %.2f' % (No_Veh, Speed, Acceleration, Length))

# Queue length
# Syntax to get the data:
#   QueueCounter.AttValue('QLen(sub_attribute_1, sub_attribute_2)')
#
# sub_attribute_1: SimulationRun (same as described at Vehicle Travel Time Measurements)
# sub_attribute_2: TimeInterval  (same as described at Vehicle Travel Time Measurements)
#
...
print('Average maximum Queue length of all simulations and time intervals '
      'of Queue Counter #%d: %.2f' % (QC_number, maxQ))
